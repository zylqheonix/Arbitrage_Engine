  ...
  ws.async_read(*buffer, [buffer, &ws, on_quote](beast::error_code ec, std::size_t) {
    if (ec) {
      if (ec == websocket::error::closed) {
        std::cerr << "[binance] websocket closed\n";
        return;
      }
      if (ec == boost::asio::error::operation_aborted) {
        std::cerr << "[binance] read cancelled\n";
        return;
      }
    ...
    try {
      json::object const result =
          json::parse(beast::buffers_to_string(buffer->data())).as_object();
      buffer->consume(buffer->size());

      // SUBSCRIBE ack: {"result":null,"id":1} — not a bookTicker payload.
      ...
      quote.best_ask = std::stod(std::string(result.at("a").as_string()));
      quote.best_bid = std::stod(std::string(result.at("b").as_string()));
      quote.bid_qty = std::stod(std::string(result.at("B").as_string()));
      quote.ask_qty = std::stod(std::string(result.at("A").as_string()));

      auto const now_sys = std::chrono::system_clock::now();
      quote.timestamp_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
